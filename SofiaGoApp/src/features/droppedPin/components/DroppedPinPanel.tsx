...
export const DroppedPinPanel: React.FC<Props> = ({ pin, onClose, onSaveFavorite, onBuildRoute, onEditLocation, primaryActionLabel, bottomOffset }) => {
    const { height } = useWindowDimensions();
    const resolvedBottomOffset = bottomOffset ?? Math.min(Math.max(height * 0.1, 72), 124);

    return (
        <View style={[styles.panel, { bottom: resolvedBottomOffset }]}>
...
const styles = StyleSheet.create({
    ...
    title: { flex: 1, minWidth: 0, fontSize: 13, fontWeight: '600', color: '#0F172A' },
    closeBtn: { width: 32, height: 32, borderRadius: 16, backgroundColor: 'rgba(248,250,252,0.72)', alignItems: 'center', justifyContent: 'center', flexShrink: 0 },
